...
def validate_available_inputs_on_screen(ssh, device_ip, log_callback=None, screenshot_folder='screenshots', reference_folder='reference_inputs', input_names=None, right_key_callback=None, max_icons=12):
    ...
    """
    import time
    from config_screenshot import rpc_url
...
def _take_screenshot_with_timeout(ssh, screenshot_name, device_ip, log, screenshot_folder, after_reboot, start_time):
    ...
    try:
        ...
        try:
            ...
            # Verify image using layout-based validation instead of text extraction
            try:
                from screen_validator_lightweight import LightweightScreenValidator
                
                # Validate the screenshot to determine which screen it is
                # By default, exclude Factory Reset screens for general screenshot validation
                # (Factory Reset validation is done separately in screen_validation method)
                log(f"🔍 Validating screenshot using layout-based image matching...")
                ...
                log(f"✓ Screen validation complete: {screen_detected} ({confidence:.2%} confidence)")
