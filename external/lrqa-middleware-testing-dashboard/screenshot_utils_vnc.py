...
def get_vnc_port_for_device(device_ip, tunnel_mode=False, tunnel_host='localhost'):
    ...
    Returns:
        tuple: (host, port)
    """
    from config_deployment import VNC_PORT_MAPPING, DEFAULT_VNC_PORT, TUNNEL_MODE
    ...
    try:
        ...
        if validate_image:
            try:
                ...
                # Perform layout-based screen validation
                try:
                    from screen_validator_lightweight import LightweightScreenValidator
                    
                    log(f"🔍 Performing screen validation...")
                    ...
                    log(f"⚠ Screen validation skipped: {val_err}")
...
def compare_screenshot_methods(device_ip, device_name, iteration, log_callback=None):
    ...
    import paramiko
    from config_devices import get_device_by_ip
