...
METHOD_EXECUTION_TIMES = {
    ...
    'deepsleep_maintenance_wakeup': 750,  # DeepSleep Maintenance Wakeup (8 steps): STEP1(10s) + STEP2(10s) + STEP3(5s) + STEP4(120s poll for offline) + STEP5(90s poll for recovery) + STEP6(120s reboot) + STEP7(180s poll for DEEPSLEEP) + STEP8(150s wake+HOME validation) + overhead = ~12.5 min per iteration
    ...
    'maintenance_CURL_deepsleep_wakeup': 3120,  # OPTIMIZED: Maintenance + CURL DeepSleep (immediate injection) + Wakeup: 45-65m maintenance (3000s) + 2m deepsleep via curl (120s) = ~52m total (saves ~20m vs standard method)
