...
# Recovery is temporarily disabled per request
...
def add_html_result(iteration, phase, status, details, screenshots="", logs="", device_ip=None, method=None):
    ...
    test_execution_service.add_result(iteration, phase, status, details, screenshots, logs, device_ip, method)
...
def list_sequences():
    ...
    try:
        ...
        for seq in sequences:
            ...
            # Show if user is admin or if sequence belongs to user's team
            if is_admin or (user_team and seq_team == user_team):
...
def get_jobs():
    """List jobs with optional filters - includes both current jobs and historical test results"""
    try:
        ...
        # Get today's UTC date for comparison
        from datetime import datetime as dt
        today_utc = dt.now(timezone.utc).strftime('%Y-%m-%d')
        
        # If no current jobs and no filters, include historical test results (only for PAST dates, not today)
        if not jobs_data and not user_id and not device_ip:
            try:
                # Load and group historical test results into proper executions
                test_results = load_test_results_history()
                if test_results:
                    # Filter out today's results (only show past dates)
                    past_results = []
                    for result in test_results:
                        result_timestamp = result.get('timestamp', '')
                        result_date = result_timestamp.split('T')[0] if 'T' in result_timestamp else result.get('date', '')
                        
                        # Only include PAST dates, not today
                        if result_date < today_utc:
                            past_results.append(result)
                    
                    # Group results into executions
                    if past_results:
                        # Get current username for attribution
                        current_username = 'Unknown User'
                        if hasattr(current_user, 'name'):
                            current_username = current_user.name
                        elif hasattr(current_user, 'user_id'):
                            current_username = current_user.user_id
                        
                        executions = group_test_results_into_executions(
                            past_results,
                            current_user_id=current_username
                        )
                        
                        # Convert executions to job format and apply filters
                        for execution in executions:
                            # Apply status filter if provided
                            if status and execution.get('status') != status:
                                continue
                            
                            jobs_data.append(execution)
            except Exception as e:
                print(f"⚠️  [API] Error loading and grouping historical results: {str(e)}")
                import traceback
                traceback.print_exc()
...
if __name__ == '__main__':
    ...
    print(f"Host: 10.0.0.123")
    print(f"Port: 11078")
    print(f"{'='*60}\n")

    app.run(debug=debug_mode, host='10.0.0.123', port=11078, use_reloader=use_reloader)
