...
# Import AI-based validator (V2.0) with fallback
try:
    from services.ai_screen_analyzer import AIScreenAnalyzer
    AI_VALIDATION_AVAILABLE = True
except ImportError as e:
    print(f"⚠️  AI Screen Analyzer not available: {e}")
...
_ai_validator = None
_lightweight_validator = None

def get_ai_validator():
    """Get or create AI validator instance"""
    global _ai_validator
    if _ai_validator is None and AI_VALIDATION_AVAILABLE:
        _ai_validator = AIScreenAnalyzer()
    return _ai_validator

def get_validator():
    """Get or create lightweight validator instance (fallback)"""
    global _lightweight_validator
    if _lightweight_validator is None and SCREEN_VALIDATION_AVAILABLE:
        _lightweight_validator = LightweightScreenValidator()
    return _lightweight_validator
...
def validate_screenshot(screenshot_path: str, expected_screen: str, 
    ...
    Uses AI-based validation with fallback to lightweight validator.
    ...
    """
    # Try AI-based validation first (V2.0)
    if AI_VALIDATION_AVAILABLE:
        try:
            ai_validator = get_ai_validator()
            if ai_validator:
                result = ai_validator.analyze_screenshot(
                ...
                if result.get('success', False):
                    return {
                        'valid': result.get('device_matched', False),
                        'confidence': result.get('confidence', 0.0),
                        'expected': expected_screen,
                        'detected': result.get('detected_screen'),
            ...
            print(f"⚠️  AI validation error: {e}, falling back to lightweight...")
    ...
    # Run lightweight validation as fallback
    ...
    return {
        ...
        'method': 'PIXEL_MATCHING_FALLBACK'
    }
