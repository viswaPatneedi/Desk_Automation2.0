...
class ResultsController:
    ...
    def get_results(self):
        ...
        try:
            ...
            job_iterations_map = {j.job_id: j.iterations for j in jobs if j.iterations}
            ...
            # Calculate missing iteration counts from actual results
            # This handles cases where job data doesn't exist in the system
            for job_id in [r.get('job_id') for r in enriched_results if r.get('job_id')]:
                if job_id not in job_iterations_map:
                    # Find max iteration number for this job
                    job_results = [r for r in enriched_results if r.get('job_id') == job_id]
                    if job_results:
                        max_iteration = max([int(r.get('iteration', 0)) for r in job_results if r.get('iteration')], default=0)
                        if max_iteration > 0:
                            job_iterations_map[job_id] = max_iteration
            ...
            for r in enriched_results:
                ...
                if len(methods_in_iteration) >= 1:
                    ...
                    summary_card = {
                        ...
                        'total_iterations': job_iterations_map.get(r.get('job_id'), '?'),
                        'date': r.get('date'),
            ...
            # Use device/iteration/job_id/date as the primary key (don't include name to catch all variants)
            deduped_results = {}
            ...
            for result in consolidated_results:
                ...
                # Primary dedupe key: device/iteration/job_id/date
                primary_dedupe_key = (device_ip, iteration, job_id or 'NO_JOB', date)

                if primary_dedupe_key not in deduped_results:
                    deduped_results[primary_dedupe_key] = result
                    continue

                existing = deduped_results[primary_dedupe_key]
                
                # Skip duplicate if it has UNKNOWN status and existing has valid status
                existing_status = existing.get('status', 'UNKNOWN')
                result_status = result.get('status', 'UNKNOWN')
                
                if result_status == 'UNKNOWN' and existing_status != 'UNKNOWN':
                    # Skip this duplicate - it has unknown status
                    duplicates_removed += 1
                    print(f"[DEDUP] Removed UNKNOWN duplicate: {primary_dedupe_key}")
                    continue
                    
                if existing_status == 'UNKNOWN' and result_status != 'UNKNOWN':
                    # Replace existing with this one - it has valid status
                    deduped_results[primary_dedupe_key] = result
                    duplicates_removed += 1
                    print(f"[DEDUP] Replaced UNKNOWN with valid: {primary_dedupe_key} ({result_status})")
                    continue

                # Both have same status level - merge them
